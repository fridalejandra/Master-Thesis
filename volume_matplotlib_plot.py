...
# ## Read in area and thickness files using xarray

# In[2]:


#Reading files with xarray 
siv_file = '/Users/fridaperez/Developer/repos/local_repo/SIT_25000/SIT_SH_Filled-316x332.nc'  # prepping path for xr
data_siv = xr.open_dataset(siv_file) # opening sit for xarray


# In[3]:

# Here we want to read the SIT files using the nc package
siv_nc = nc.Dataset("/Users/fridaperez/Developer/repos/local_repo/SIT_25000/SIT_SH_Filled-316x332.nc") #reading SIT from nc
#reading in SIT variables 02-11
time = siv_nc.variables['time'][:]  #sit time
lat=siv_nc.variables['latitude'][:]
lon=siv_nc.variables['longitude'][:]
siv=siv_nc.variables['siv'][:,:,:]
...
# the relevant indices
yeardata = data.volume.groupby('time.year').mean('time',keep_attrs=True)
...
# Extract the january months by selecting 
# the relevant indices
mothsdatas = data_sit.SIT.groupby('time.month').mean('time',keep_attrs=True)
